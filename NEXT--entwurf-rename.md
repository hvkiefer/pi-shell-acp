...
>   - **미커밋 — GLG 커밋 지시 대기:** 변경 7파일(backend-adapter/config/backend + check-acp-config/session-reuse + docs/acp-backend-rail.md + 이 NEXT). 커밋(가드+seam+docs+NEXT 한 커밋)은 GLG 승인 후 `commit` 스킬로, push=GLG. + 부수: models.ts getModels deprecated(L32/69) 별도 chore.
