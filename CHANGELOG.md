...
## Unreleased — v2-only: subtract ACP/v1 (2026-06-17, not yet tagged)

> **v2-only subtraction lane** (accumulating; no tag cut yet). With the entwurf v2 dispatch substrate proven in 0.11.0, this release **removes the layers it was built to replace**: the ACP provider/backend surfaces and the v1 entwurf entrypoints. What remains is the pi-native v2 substrate alone — three transports over *existing* garden citizens (control-socket send / spawn-bg resume / meta-mailbox enqueue) plus the meta-bridge. Strategy was **subtractive** (delete with the gates as guardrails, so anything load-bearing fails loud) rather than additive cherry-picking. **The name is preserved on purpose** — `pi-shell-acp` → `entwurf` rename is deferred to Phase B; this release ships a working v2-only tree whose package name still says ACP, with a branch-status banner in `AGENTS.md` flagging the name↔content gap. **Documented capability cut:** fresh sibling minting (the v1 `entwurf.ts` "spawn a NEW task" verb) is intentionally *not* replaced here — every v2 transport targets an already-identified citizen, so "create a new sibling from nothing" is an explicit, documented hole (not a silent regression) pending a later `spawn-fresh` lane. Verified green throughout: `pnpm check` EXIT 0 and `LIVE=1 release-gate` MUST 6/6 on thinkpad, with both delivery rails (Claude↔GPT) exercised live.

### Removed

- **ACP provider/backend surfaces.** Root ACP sources (`index`/`acp-bridge`/`event-mapper`/`engraving`/`pi-context-augment`), the v1 `entwurf.ts`, ACP-only scripts (`transcript-poison-smoke`, `compaction-policy-smoke`), ACP deps (`@agentclientprotocol/*`, `@zed-industries/codex-acp`, `@anthropic-ai/sdk`), the `plugins/openclaw` importer (+ `check:plugins`, workspace glob), and ~16 ACP functions / 18 cases from `run.sh`. Global pnpm ACP packages (`claude-agent-acp`/`codex-acp`/`pi-acp`) + dangling bin shims removed from the device; `pi` (earendil) and `codex` (`@openai/codex`, native) unaffected.
- **v1 entwurf entrypoints.** MCP tools `entwurf_send`/`entwurf`/`entwurf_resume`, pi-native `entwurf_send`, the `spawn_async_resume` control RPC, the `/entwurf-send` startup command, `lib/entwurf-v2-only.ts` (the v1 hard-refusal gate, now moot) and `lib/entwurf-async.ts` (v1 async-resume launcher, orphaned). v1 gate/async check scripts dropped with their run.sh cases and check-chain entries (deletion-coupling rule: function + case + npm script + chain together).
- **Dead v1/ACP smokes.** Purged from `run.sh` and from the release-gate MUST tier (6 smokes whose functions were already deleted were still invoked → fake RED → gate falsely NOT-releasable), plus orphan `_single` workers and `smoke-async-resume.sh`.

### Changed

- **release-gate rescoped to the v2-native live floor.** `smoke-session-id-name` + RGG retargeted from hardcoded ACP `--provider pi-shell-acp` to a native target (`PI_SHELL_ACP_LIVE_TARGET`, default `openai-codex/gpt-5.4`) with isolated `auth.json`. **repo-under-test decision:** LIVE smoke is the code under test, not a deployment smoke — every resident `pi --entwurf-control` spawn loads only this checkout via `--no-extensions -e $REPO`. MUST tier is now 6 live subcommands; `xt-tool-surface`/`session-messaging`/`sentinel` dropped from the floor and marked `[LEGACY — broken on v2-only]` (fail-loud), pending a v2 rewrite.
- **env sender seam.** `updateSessionEnv` now sets `PI_AGENT_ID=<provider>/<model>` alongside `PI_SESSION_ID` (symmetric set/delete), so a spawned MCP child inherits a correct sender identity; caller-side already had the `ctx.model` fallback. Agent-id format moved `pi-shell-acp/<model>` → `<provider>/<model>`.
- **`entwurf_peers` unified onto the two-rail fact surface.** The pi-native tool previously scanned only the control-socket rail (`getLiveSessions`), while the MCP bridge already merged the meta-record rail; both now route through the same `listEntwurfFacts` + `renderEntwurfPeers` fence, so meta-mailbox citizens (including self-fetch claude-code receivers) are discoverable too. **Bounded output:** each section shows the latest 32 entries (`older entries omitted`) and the full JSON payload is no longer pasted into human text (kept in `details`), fixing a 63KB → 4.3KB tool-result overflow.
- **`AGENTS.md` rewritten for the v2-only surface** — truthful baseline (pi-native dispatch substrate + meta-bridge, 8 hard rules, real v2 verification commands) with a branch-status banner for the name↔content gap.

### Fixed

- **`--emacs-agent-socket` flag revived.** The flag was registered/parsed by the now-deleted ACP `index.ts`, so `pi … --emacs-agent-socket server` was rejected as unknown. Re-registered in `entwurf-control.ts`, exporting `PI_EMACS_AGENT_SOCKET` from the resident's own env (no ACP child needed), independent of `--entwurf-control`.

### Deferred

- **Fresh sibling minting** → a later `spawn-fresh` lane (documented capability cut, not silent).
- **Phase B (rename `pi-shell-acp` → `entwurf`)** + routing residue (`getRegistryRouting` hardcode, `resolve-acp-bridge.ts` orphan, `mcp/index.ts` description), README rewrite, and stale doc-comment cleanup — all carried in `NEXT--v2-only.md`.
