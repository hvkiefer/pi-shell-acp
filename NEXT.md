...
## NOW — 2026-06-16 KST — 0.11.0 컷 ★보류★: v2 recordless-pi bad-target 버그(release-blocker급 honesty gap) 검토 중. 문서 정합 sweep은 닫힘.

> **2026-06-16 detour (GLG+GPT5.5+Claude, release-gate 보류):** entwurf_peers가 live로 보여주는 record-less pi control-socket 세션을 `entwurf_v2`가 `bad-target`으로 거부한다. fact surface ↔ v2 resolver 계약 불일치. 재현: live peer `20260616T094836-a32c30`(piar로 직접 띄움, socket alive, meta-record 없음) → `entwurf_v2` `bad-target(liveness n/a)` 거부 / `entwurf_send` 정상 전달.
> - **근원:** `pi-extensions/lib/entwurf-v2-production.ts:229` resolveTarget — `if (!io.metaRecordExists(gid)) return {identity:null}` → decider가 `bad-target`. frozen table은 "alive pi → control-socket send"라 약속하나, record gate가 record-less pi socket에 대해 그 약속을 깬다.
> - **왜 smoke가 못 잡나(coverage gap):** `smoke-entwurf-v2-spawn-resume-live`(run.sh:21)는 **"mint a backend=pi meta identity → seed a REAL citizen"** — pi target에 meta-record를 심고 시작. matrix-live C1도 record-backed. operator가 직접 greet한 record-less live pi socket을 v2 target으로 쓰는 케이스를 덮는 smoke가 없다.
> - **fix 방향(검토용, GLG/GPT 결정 대기):** (A) 더 좋은 fix — record-less in-domain(pi) target은 socket-identity로 fallback해 v2가 control-socket send 하도록(frozen table·entwurf_send와 일치). (B) 최소 honesty fix — entwurf_peers에 `v2Target=false`/recordless-control-only 표기 + entwurf_v2 문서에 "record-backed citizen만 target" 명시. 단 0.11.0 headline이 "v2 live pi send"면 B만으로는 headline 약화 → A 권장. **어느 쪽이든 record-less live-pi-socket을 v2 target으로 covers하는 새 smoke 필수.**
> - **2순위 detour(GC):** meta-session record 117개 누적. entwurf_peers default를 live+recent+current-cwd로 제한, dormant/meta는 옵션(`--all`/`includeDormant`), stale receiver/sender marker·read mailbox body는 GC, meta-record는 archive/TTL/lastSeen 정책. listing hygiene + stale meta GC.
