...
- 0.5.0은 **guard split only**. recap engine, compact→new-session handoff, hidden transcript hydration 금지.

## Next Steps

### 1. Source audit before edit

Check current guard sites:
...
- #10 peer-handle / sessionId-only carrier RFC implementation

## Parked, not current

- [#10](https://github.com/junghan0611/pi-shell-acp/issues/10): collapse `entwurf` / `entwurf_send` / `entwurf_resume` into one peer-contact verb with caller-held authority.
- #9 follow-ups: runtime `resumeSession` smoke; non-recording `demo/demo.sh` smoke variant.

## Completion rule

When 0.5.0 guard split is done, replace this file with the next actual priority. Do not append release logs here.
