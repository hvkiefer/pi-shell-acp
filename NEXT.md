...
**▶ 다음 한 걸음 (구현 세션 진입점 — 버킷 A + 3D-2 완료, 다음 = 3D-3):**
- **(main track) step 3D-3 = capability-backed metadata.** live descriptor 소비처를
  `META_BACKEND_DESCRIPTORS` → capability registry(3C, `pi/entwurf-capabilities.json`)로 전환. 기존 3 backend
  drift guard **유지**, record 안 wakeMode 제거 **준비**(제거 자체는 3D-4). **3D-2와 같은 패턴(Fable/GPT 권고):
  게이트 먼저 → additive 배선 → 기존 drift guard 안 깨짐.** 정찰은 진입 시(현 소비처 = `META_BACKEND_DESCRIPTORS`,
  아래 "현 authority 위치" 참조). 이후 3D-4(v2 writer/upsert + 끊을 지점 ②).
- **3D-2 ✅ 완료** = `79b3c98`. live receipt dual-write: `enqueueMetaMessage`/`readMetaInbox`가
  `record.delivery.*`(v1 home) stamp **유지** + mailbox `state.json`(3B) dual-write 추가, byte-identical
  (같은 `now`/`gardenId`/`mailboxDir`; markEnqueued/markRead/stampMailboxReceipt 모두 isoNow). 게이트
  `check-meta-mailbox-dualwrite` 15 + smoke-meta-mailbox green. additive only(delivery 제거/v2 upsert/capability
  전환 X). **비차단(3D-4 게이트 update 시 참고):** read-side가 drift'd `state.json`에 throw하면 메시지는 이미
  `.read` archive라 재호출 0건 반환 — body는 디스크에 남아 수동복구 가능하나 API로는 안 돌아옴. no-rollback
  스코프의 내재적 결과이고 drift 가드는 실제 corruption에서만 발화 = 수용(Fable 검수 2026-06-10).
...
> **다음 = step 3D-3(capability consumer 전환).**
>
