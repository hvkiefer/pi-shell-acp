...
- **`entwurf_resume` native default flipped `sync` → `async` (Phase A of the async-resume regression repair).** Restores the pre-Phase-0.5 native behavior where long-running resumes (review / research / build) detach and deliver completion as a followUp message instead of blocking the parent turn. The async branch (`pi-extensions/entwurf.ts:929-1137`) was already alive and unchanged — only the schema `default` and the `params.mode ?? "..."` runtime fallback were inverted. This completes the 0.7.0 axis symmetry: `ad4413e` (2026-05-19) flipped `entwurf` spawn to `async` default for the same reason ("review / research / build dominate; blocking the parent turn for >30s reads as 'stuck'"), but left resume on the Phase 0.5 `sync` default — producing the most awkward state where short spawn (often <5s) detached while long resume blocked. Sync stays available as `mode="sync"` opt-in for short status-check resumes (<5s). VERIFY.md §0A now records the explicit `mode="sync"` requirement for verification turns where the operator needs inline answers rather than detached followUp delivery. The MCP bridge surface (`mcp/pi-tools-bridge/src/index.ts`) still exposes sync-only; Phase B is tracked in NEXT.md as the remaining half — gated by `buildSendSenderEnvelope` replyable discriminator (same pattern as `entwurf_send` `wants_reply` rejection), requires async-launcher extraction into `entwurf-core.ts` plus an `entwurf-control.ts` `spawn_async_resume` RPC so the MCP handler can delegate back into the native code path instead of cloning it (preserves the "this bridge is not a second harness" invariant). The pre-Phase-0.5 baseline pattern — "sync spawn anchors the 담당자, async resume frees the parent turn for the long body of work" — returns on the native axis with this change.
