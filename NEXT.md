...
- ▶ **NEXT (in order):** (1) `-xt` ACP-backend exclude-tools policy fix (Step 1e — release-blocking; decide per-backend mapping vs fail-fast, **needs GLG decision**); (2) Step 2 dep bump (verified by release-gate); (3) Step 3 Opus 4.8; (4) Step 4 docs; (5) Step 5 cut. **release-gate stays RED until `-xt` is green; GLG closes the final 0.8.0 authorization.**
