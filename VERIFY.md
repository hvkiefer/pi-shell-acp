...
0.8.1 pre-cut evidence (2026-05-31): `/tmp/pi-tmux-release-gate-0811c.log` — `PASS=15 FAIL=0 SKIP=0`, invoked from the repo cwd against `/tmp/psa-release-gate-0811c.Z7L4VB`. This floor now includes the package-install routing live gate `smoke-installed-entwurf-acp (#29)` ahead of the Entwurf runtime matrix. The cwd-hygiene cross-check remains part of the evidence: `sentinel-20260531-182435.json`, `smoke-async-resume-20260531-181934.json`, and `session-messaging-smoke-20260531-182737.json` all record scratch-session paths rather than `--home-junghan-repos-gh-pi-shell-acp--` repo-session paths.
