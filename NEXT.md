...
**구현 순서 (봉인판, 오푸스 인계):** ~~① NEXT stale 정정 → ② stale 주석 → ③ contract 확장 → ④⑤ adapter leaf+구현 → ⑥ decider rail 일괄 착지~~ **①~⑥ 완료(b8dc731..524b0be, 2026-07-04 GPT 코드리뷰 R1~R9 전 축 PASS — matrix에 antigravity row 불추가·tripwire 유지가 정답 판정)** → ⑦ `entwurf_register_native`(봉인 5, 진행 중) → **⑦.5 Q12 수정요구(⑨ LIVE 전 필수): `NativePushRunner.exec`에 `timeoutMs` + real runner `execFile` timeout(raw parity 8s) — timeout은 probe→indeterminate/send→throw(executor 1회 re-probe 규율 유지), 게이트에 no-hang 단언** → ⑧ agy install 3종 + `smoke-agy-install-state`(봉인 7·8) → ⑨ LIVE smoke + DELIVERY.md shipped 갱신(봉인 8). 선택 보강(blocker 아님): Q13 native-push 파일군의 mailbox 원자 import-금지 source guard, Q14 adapter.ts root/scripts tsconfig 이중 포함 정리(green이므로 문서화로 충분). **⑨ 선행조건이던 오라클 agy 배선은 수선 완료**(agent-config `mcp_config.server.json` command → repo 직결, 실존 확인).
