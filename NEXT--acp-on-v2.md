...
- **S2 컷 (≈4~5 Opus + 2~3 GPT)** — 순서가 안전장치 (**현재: S2c까지 DONE, 다음 S2d**):
  ...
  - ~~**S2c** 이벤트매핑 + `streamSimple` 실 backend 교체(stub 제거)~~ ✅ **DONE (2026-06-18, Opus 3rd)**: S2c-1 event-mapper+context+gate / S2c-2 streamShellAcp+runtime preflight+stub제거+live provider smoke. dangling-timer pi-hang 버그 잡음. GPT(`73b44d`) PASS. **미커밋분은 GLG commit/push 대기**.
