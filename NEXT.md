...
- ✅ **DONE this session:** Step 1 gate hardening — 1b (fold `check-model-lock` + `verify-transcript-poison` into `pnpm check`); 1c (`check-dep-versions` 6→12 assertions incl. the three pi devDeps + `check-pack-install` peer pin); 1e (`./run.sh release-gate` skeleton, `-xt` fail-closed stub); 1f (prepublishOnly already static+pack) — commits `12d55eb`, `6592c5f`. Step 2b #26 no-auth sentinel + `check-auth-boundary` guard — commit `ef56394`, **verified live** (release-gate `PASS 7→12`, 5 gates recovered, `No API key` count 0).
- ▶ **NEXT (new session, in order):** (1) sentinel cell 4/5/6 — add `--entwurf-control` to the ACP parents in `sentinel-runner.sh`, re-run to confirm it's a test-setup gap; (2) sentinel cell 2 — best-effort async completion delivery (`entwurf.ts:853` + `entwurf-async.ts:351`); (3) `-xt` ACP-backend exclude-tools policy fix; (4) Step 2 dep bump (verified by release-gate); (5) Step 3 Opus 4.8; (6) Step 4 docs; (7) Step 5 cut. **release-gate stays RED until `-xt` + sentinel are green; GLG closes the final 0.8.0 authorization.**
...
**Sentinel root-cause analysis (2026-05-29, code + pi-mono 0.77.0 source). Two distinct, separable causes — NOT one bug. Implementation is for a NEW session; this round is analysis + NEXT alignment only.**
