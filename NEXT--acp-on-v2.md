...
- **Next = S2d (billing carrier + session reuse + engraving + first-user augment)**: 이제 안전하게 진입 가능(raw pipe+overlay+event mapping 다 살아있음). 핀1 billing carrier(engraving SHORT/empty=`_meta.systemPrompt`, 순수함수) + session store/signature/reuse(bootstrapPath reuse|resume|load|new) + first-user message augment(bridge identity+~/AGENTS.md+pi intro, de-dup). 상세 = §S2-scout 핀1 + oracle A~C + oracle G·H.
  ...
  - **S2d-1b 설계 굳음 (GPT `73b44d` PASS)**: **bootstrapPath(history source: new|reuse|resume|load)와 lifecyclePolicy(`process-scoped` vs `turn-scoped`) 분리** — 두 축 독립. in-memory child/connection reuse = **long-lived process 한정**(resident/RPC). **`pi -p` one-shot = 항상 new+teardown**(S2c 동작 유지 — stdio handle이 exit를 pin하므로 exit-hook 정리는 hang 회귀 위험; 첫 cut에서 persisted resume/load for one-shot도 금지, 미래 turn-scoped persisted는 별도 follow-up). **carrier-first 분리**: S2d-1b는 carrier 부재(`appendSystemPrompt` empty) → signature=model당 상수 → reuse lifecycle만 검증, engraving carrier(`_meta.systemPrompt` SHORT 순수)+first-user augment=**S2d-1c**. **sub-cut**: 1b-1(deterministic: `bridgeConfigSignature`/`contextMessageSignatures`/`isCompatible`(sig+prefix-compat)/`decideBootstrap`/record store, 순수+temp-dir gate) → 1b-2(live: in-memory session map + reuse wiring + long-lived 2턴 reuse smoke + one-shot S2c exit0 유지 확인). 1b-1 게이트: ① modelId guard를 순수 decision에도(record에 modelId; existing alive mismatch→fail-loud throw) ② prefix-compat 불일치/compaction/edited→new+full transcript ③ carrier drift→incompatible(existing close/persisted delete) ④ augment는 signature 절대 미포함(1c pending one-shot).
    ...
    - **다음 = S2d-1b-2b (제일 위험 — child lifecycle 전환, GPT 구현 전 재정렬 필수)**: process-scoped에서만 in-memory `bridgeSessions` Map에 child/connection 보관 + reuse 턴이 기존 connection에 `buildAcpPrompt(ctx,path)` delta 전송 + resume/load(initialize capability 확인, 없으면 new) + 턴 성공 후 record write(ctxSigs 갱신) + best-effort cleanup(`process.once('exit')` sync group kill / SIGINT·TERM best-effort / `child.unref`+stdio unref, 단 control socket이 loop 유지). **live smoke 2축 필수**: long-lived resident/RPC 2턴 reuse(2턴째 delta인데 backend가 1턴 history nonce 기억) + **one-shot provider smoke exit0 유지**(reuse 도입이 `pi -p` exit-clean 안 깸). source/behavior check: streamShellAcp가 hardcoded `contextToAcpPrompt` 아니라 `buildAcpPrompt(context, bridgeSession.bootstrapPath)` 호출.
- **잠긴 방향 (굳음)**:
...
- **S2 컷 (≈4~5 Opus + 2~3 GPT)** — 순서가 안전장치 (**현재: S2c까지 DONE, 다음 S2d**):
...
- 즉 **어느 컷에서도 Opus·GPT 둘 다 동시에 새로 가지 않는다.** 기본: GPT 세션(현 `20260618T155548-73b44d` — `be0e35`는 S1 후 퇴근, `441eab`가 S2-scout~S2c 검수 후 rotate, `73b44d`가 S2c 최종 PASS + push-green 재확인 + **S2d 검수 anchor**)을 살려두고 Opus만 re-mint. (S0/S1/S2a~S2c가 그 키트로 깨끗이 re-mint된 게 증거 — Opus가 NEXT+AGENTS+GPT 리뷰만으로 re-mint돼 매 컷 완주.) 깨지는 경우 = 둘 다 새로 갈 때 → GPT 닫기 전 review-trail을 NEXT/botlog에 남길 것.
...
# 넘으면 안 되는 선
- **다음 coding = S2d — S0·S1·S2a·S2b·S2c DONE(GPT PASS).** §S2-scout 핀이 하드제약. S2d = 핀1 billing carrier(engraving SHORT/empty=`_meta.systemPrompt`, 순수함수 — drift 시 매턴 rebuild) + session store/signature/reuse(bootstrapPath reuse|resume|load|new, `bridgeConfigSignature`) + first-user message augment(bridge identity+~/AGENTS.md+pi intro, entwurf-spawn de-dup). **⛔ 핀1 위반 주의**: 캐리어가 SDK-default서 material하게 멀어지면 구독 billing→metered→HTTP400. 캐리어=작게, 리치컨텍스트는 first-user prepend. provider path는 이미 열림(S2c) — 이제 streamSimple 안에서 carrier/reuse만 보강. **⛔ 핀4 위반 주의**: reuse 도입 시 전체 transcript 재주입 금지(중복) — prompt 범위를 bootstrapPath별 분기(new=transcript / reuse·resume·load=first-user delta), augment는 new 첫 prompt 1회만(§Next 핀4 + oracle C). socket/peers/`entwurf_v2`/v2 core 손대기 금지. **v1 `entwurf`/`entwurf_send`/`entwurf_resume` 절대 부활 금지.**
