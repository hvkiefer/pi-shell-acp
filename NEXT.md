...
| **F3** High | `gcStaleSockets`가 **timeout에도 unlink** → 부하로 멈칫한 live 소켓 영구삭제 → 이후 probe 전부 dormant → live 세션 resume = identity split. "probe는 계산만" 교리 자기위반 | `entwurf-control.ts:288-310`(timeout·error 둘 다 false), `282-285`(`!alive`→unlink) | A/B |
...
1. **F1 = caller intent 선언.** liveness는 transport만, outcome 계약 불변. 4칸표:
   | intent | live | dormant |
   |---|---|---|
   | **fire-and-forget** | send, ack only | resume로 몰래 바꾸지 말고 **reject** 또는 mailbox-wake 정의 후 허용 |
   | **owned-outcome** | **기본 reject**(wants_reply는 ownership 아님) | async resume, caller completion 소유 |
   → owned+live를 자동 send(wants_reply)로 강등 금지. dispatch 영수증(어느 경로로 갔고 caller가 뭘 기대?)을 contract 산출물에 포함.
2. **F3 = unlink 축소만(heartbeat는 2단계).** `isSocketAlive → alive|dead|indeterminate`; ECONNREFUSED/ENOENT만
   dead; timeout=indeterminate; GC는 dead만 unlink. 게이트: "timeout 소켓은 unlink 안 함."
3. **F5b = getEntry 전환 + active-prompt 탈출구.** preflight evidence에 `trustStoreEntryPath`/`trustStoreInherited`/
   ...
   direct-false outcome의 entryPath===cwd. 탈출구 = 핸들러 직접 prompt → yes면 `{trusted:"yes", remember:true}`
   (결정 6 carve-out). **undecided-defer는 0.79.1에서 작동 안 함**(Trust 2층 정정 참조).
...
- **버킷 A (지금 코드+게이트, 결정 1 완성):** F5a/c(preflight `getEntry` + evidence 3필드 + 탈출구 방향
  assertion) · Trust 2층 핸들러 `undecided`→`undefined`(abstain) 정정 + 상속-false active-prompt(`remember:true`).
  + F3 unlink 축소(여기 넣을지 B로 둘지는 구현 시 — 작아서 A 권장).
- **버킷 B (entwurf_v2 step 4-5 진입 전, 원장에 산문 아니라 결정표/스키마/게이트로):** F1 4칸표+dispatch 영수증 /
  F2 per-gid lockfile + pi 동시-resume 실측(검증원장 추가) + send-fail fallback / F4 liveness를 backend-capability
  술어로 추상화 + claude liveness 술어 Stage 1 전 한 줄 동결 / F6 entwurf_v2 TypeBox 입출력 스키마 + target 의미론
