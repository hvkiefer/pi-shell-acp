...
- **`check-bridge` backend probe now forces a tool call instead of trusting a model self-report.** The old per-backend visibility step asked the model to *list* the MCP tools it could see, with an "answer `NOT_VISIBLE` if you don't see them" escape. Claude (sonnet) has L1 self-recognition variance — it intermittently enumerates only its native tools and answers `NOT_VISIBLE` even though the pi-tools-bridge MCP tools are injected and callable, which flaked the release gate for a model-introspection quirk rather than a bridge defect. The probe now (1) forces a read-only `entwurf_self` call and anchors on the bridge-generated `agentId: pi-shell-acp/` result line, then (2) forces an `entwurf_send` call at a socketless synthetic sessionId and anchors on the cross-backend/cross-language missing-target boundary (`control socket` / `[tool:failed]` / `isError` / Korean socket phrasings). Both phases are bounded-retried. Tool-surface wiring stays deterministically proven by the direct MCP smoke + `mcp/pi-tools-bridge/test.sh` tools/list parity; this probe owns the orthogonal "a live backend child can actually call the injected tool" axis, now verified green for Claude + Codex + Gemini.
...
- **`run.sh smoke-installed-entwurf-acp`** — credential-free live counterpart that reproduces git- and npm-installed package sources in an isolated agent dir, drives the real resolver to compute each bridge `-e`, then spawns a `pi --no-extensions -e <bridge> --list-models pi-shell-acp` child and asserts the provider registers (no `Unknown provider`, no `No models matching`). Wired into `release-gate` ahead of the Entwurf live gates so a green repo-checkout gate can no longer hide a broken official install path.
