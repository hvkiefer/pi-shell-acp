...
The async path is verified by `./run.sh smoke-async-resume` (scripts/smoke-async-resume.sh, added in 0.7.6). The live smoke covers the replyable MCP → `spawn_async_resume` control-RPC → native async launcher path across Claude, Codex, and Gemini (Hard Rule #7 three-backend equality): the backend performs an MCP `entwurf` sync-only spawn, then calls `entwurf_resume(mode="async")`; the gate asserts an immediate async ack and a later successful `🏁 resume … completed` followUp. `❌ resume` is fail-closed, not PASS. The omitted-mode conditional default (replyable → async, external → sync) is pinned by the deterministic `./run.sh check-async-resume-gate` (16 assertions) in the `pnpm check` chain; the live smoke uses explicit `mode="async"` so model prompt-following cannot hide the async branch. The 0.8.0 pre-cut baseline is `/tmp/smoke-async-resume-20260529-164342.json` (6 PASS / 0 FAIL / 0 SKIP, including `A.async.claude-sonnet-4-6`). It is NOT in the deterministic `pnpm check` chain because it requires live ACP turns and a small token spend (~$0.10–$0.30). Run it before any release that touches the entwurf surface, and after any change to `pi-extensions/lib/entwurf-async.ts`, `mcp/pi-tools-bridge/src/{index,resume-mode}.ts`, or `pi-extensions/entwurf-control.ts` (the entwurf-control RPC dispatcher).

### Release gate — 0.8.0 floor

For release cuts, the canonical command is now `./run.sh release-gate <scratch-project-dir>` with no `--allow-skip-gemini`. It runs the full deterministic floor (`pnpm check`) plus every live per-invariant gate and reports one PASS/FAIL/SKIP summary. A green gate is necessary but not sufficient; GLG still authorizes the cut.

0.8.0 pre-cut evidence (2026-05-29): `/tmp/release-gate-0.8.0-final2-20260529-164313.log` — `PASS=14 FAIL=0 SKIP=0`, invoked from the repo cwd against `/tmp/pi-shell-acp-release-gate-20260529`. The cwd-hygiene fix was part of the evidence: latest artifacts contain no `--home-junghan-repos-gh-pi-shell-acp--` session paths; `sentinel-20260529-164759.json` records six scratch session files, and `smoke-async-resume-20260529-164342.json` records the direct-stdio async session under the scratch session dir.
