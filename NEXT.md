...
**▶ 다음 한 걸음 (구현 세션 진입점 — fact-provider slice 1·2·3·4a·4b·**4c 전부 완료**(4c=커밋 대기, 아래), step 4 fact-provider **코어 완결**. 다음 = enrich(get_info cwd/model/idle) 후속 slice → F-mailbox amendment → step 5 entwurf_v2 dispatch):**
> **slice 4c 완료 (2026-06-11 세션, 커밋 대기 = GLG):** GPi+Fable 양 검수 GO 후 게이트-퍼스트로 구현. 2커밋 분할 권장:
> - **커밋 A (socket-axis 보강)** = `socket-discovery.ts` `scanSocketProbes` 반환을 `SocketScanResult{probes, symlinkedGardenIds, malformedNames, dirError}`로 확장 + symlink 거부(P1: gid-shaped `*.sock` symlink는 probe 안 함 — citizen은 dead 강등, record-less는 drop; correlation-authority 위조 차단) + malformed name 가시화(P3) + dir-read ENOENT=empty/else=surfaced(P2e②). **GPi 4c검수 Q2/P1 보강: non-ENOENT dirError 창에서는 citizen을 probe하지 않고 `indeterminate` 반환**(dir 못 읽으면 canonical path가 symlink 아님을 확인 못 하고 `connect()`는 link를 따라가므로 — "symlink never probed" invariant를 dirError 창에서도 지킴; held not stranded). `EntwurfDiagnostic` union에 3 kind 추가, `listEntwurfFacts`가 fold. 게이트 `check-socket-discovery` 18→**31**, `check-entwurf-fact-provider` 17→**27**.
> - **커밋 B (MCP surface 4c)** = 순수 `entwurf-peers-render.ts`(`renderEntwurfPeers` → text 3섹션 + payload) + `entwurf_peers` handler 배선(`listEntwurfFacts` IO deps 주입). **`sessions`는 facts의 projection**(alive pi citizen + alive socket-only, `getLiveSessions` 재호출 금지=quarantine 우회 차단, GPi 보정), socketPath=`controlSocketPath` SSOT(P2a), count=projection 길이(P3d). verdict 필드/단어 금지(JSON deep scan + text word denylist, P2e①), 빈 섹션 `(none)`·unsupported 노출·`(not enriched)`. 레거시 `getLiveSessions`/`isSocketAlive` **제거**(새 경로가 대체; wiring guard는 `\bname\s*\(` regex로 def/call 차단, 산문 멘션 허용 — GPi Q4). 게이트 `check-entwurf-peers-surface` **40** 신규. tool description은 `DISPATCH_TABLE` 의미론 정적 인용(routing 아님). `pnpm check` EXIT=0(check-pack 124 files 새 lib 포함).
> - **Fable 검수 5건 전부 닫힘**(P1 symlink / P2a socketPath SSOT / P2e① text word guard + ② dir-read 비대칭 / P3c malformed name + d count). GPi(`20260611T115213-3aa371`)·Fable(`20260611T112732-0f42b6`) 계속 가용.
> - **enrich 후속 slice(다음):** `SocketProbe`/`SocketOnlyFact` enrich(cwd/model/idle)는 현재 null 노출(probe-only 정직). `enrichSocketProbes`(alive 소켓만, `Promise.allSettled` + per-socket infoError, 전체 throw 금지)로 get_info 배선 = 별도 slice. 기존 `getLiveSessionsWithInfo`의 이 lib 전환은 additive 교리상 후순위.
> - **이월(backlog, GPi 4c검수 관찰):** 레거시 `entwurf_send` **direct socket path**(index.ts: `path.join(ENTWURF_DIR, sessionId+SOCKET_SUFFIX)` → connect)는 아직 symlink guard를 안 탄다 — 4c의 P1은 fact surface/v2 dispatch(facts 소비)에만 효과. 레거시 direct send를 완전히 닫으려면 send 전 lstat symlink 거부가 별도 후속(step 5 또는 그 전 작은 hardening). 데이터 손상 아님, 위조 listener로의 direct send 가능성만.
...
**slice 4 분해: 4a ✅(`62e9bd6` listAllMetaIdentities, check-meta-listing 13) → 4b ✅(listEntwurfFacts 조립, `entwurf-fact-provider.ts`, quarantine+diagnostics union, check-entwurf-fact-provider 17→26 in 4c) → 4c ✅(2026-06-11, 커밋 대기 = GLG; socket-axis 보강 + MCP `entwurf_peers` render `entwurf-peers-render.ts`, Fable 5건 + GPi 4c검수 Q2/P1·Q4 닫힘, check-entwurf-peers-surface 40 / check-socket-discovery 31 / check-entwurf-fact-provider 27). = step 4 fact-provider 코어 완결.**
