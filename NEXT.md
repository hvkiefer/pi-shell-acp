...
## Current state — 2026-06-11 퇴근 전

- Code baseline before this NEXT cleanup: `origin/main` at `2b339dd` (4c shipped). This document cleanup may be a local docs commit until GLG pushes.
- Latest commits:
  - `6467466 feat(entwurf): fact-provider socket-axis hardening — symlink/dir-read/malformed diagnostics`
  - `2b339dd feat(entwurf): fact-provider slice 4c — entwurf_peers facts surface + render`
- Done: step 4 fact-provider slice 1·2·3·4a·4b·4c. `entwurf_peers` now renders facts + legacy `sessions` projection from the same provider; no second socket scan.
- Verified: `check-socket-discovery` 31, `check-entwurf-fact-provider` 27, `check-entwurf-peers-surface` 40, prior full `pnpm check` green.
...
1. **Small optional follow-up:** `get_info` enrich for alive socket probes (`cwd/model/idle`, per-socket `infoError`, `Promise.allSettled`, no whole-list throw). Current `(not enriched)` is honest.
2. **Step 5 blocker:** **F-mailbox amendment** before `entwurf_v2` dispatch. Add `meta-mailbox` transport; `fire-and-forget + unsupported citizen → send/meta-mailbox/ack-only`; `owned-outcome + unsupported → reject` stays.
3. **Then step 5:** implement `entwurf_v2` dispatch consuming fact-provider + contract, with F2 per-gid lock/preflight/transport execution.
4. **Separate backlog:** pi `session_start` meta-record writer; legacy `entwurf_send` direct socket symlink guard; release-gate matrix smoke.
