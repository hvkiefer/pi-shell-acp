...
for (const intent of ENTWURF_INTENTS) {
	for (const sl of SOCKET_LIVENESSES as readonly SocketLiveness[]) {
		...
		const receipt = resolveDispatch(intent, sl);
		ok(`round-trip ${intent}/${sl}: observedLiveness === input`, receipt.observedLiveness === sl);
		if (cell.action === "reject") {
			ok(
				`round-trip ${intent}/${sl}: receipt reject matches cell`,
				receipt.ok === false && receipt.reason === cell.reason,
			);
		} else {
			ok(
				`round-trip ${intent}/${sl}: receipt allow matches cell`,
				receipt.ok === true &&
...
// ── R1: unsupported rejects distinctly — NOT folded into indeterminate ─────
for (const intent of ENTWURF_INTENTS) {
	const receipt = resolveDispatch(intent, "unsupported" as FactLiveness);
	eq(`unsupported/${intent} → backend-liveness-unsupported (distinct from indeterminate)`, receipt, {
		ok: false,
...
for (const r of TABLE_REJECT_REASONS) {
	ok(`taxonomy: table reason '${r}' is in the enum`, (ENTWURF_V2_REJECT_REASONS as readonly string[]).includes(r));
}
