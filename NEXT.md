...
**구현 순서 (봉인판, 오푸스 인계):** ~~①~⑥(b8dc731..524b0be) → ⑦ register(68170a7) → ⑦.5 Q12 timeout(31cc023) → ⑧ agy install 3종+smoke(9a69e45; N1 drift+N2 honest-label 14654b8)~~ **②~⑧ 완료(2026-07-04, 오푸스 — 각 커밋 pre-commit 전체 `pnpm check` green; 페블 실측검수 R1~R9+N1/N2 반영, Q13 source guard는 31cc023에 포함, ⑤는 ④에 통합)** → **⑨ (잔여, GLG-gated): LIVE smoke `smoke-agy-native-push-live`(register→entwurf_v2→token 원샷, `LIVE=1`+`AGY_CONVERSATION_ID`) + `DELIVERY.md` agy verified-probe→shipped 갱신.** **라이브 선행 증거 확보(2026-07-04 GPT 수동 실측, production MCP bridge surface 경유): L1~L5 PASS** — 실 agy 1.0.8 conversation에 `entwurf_register_native` create/attach 멱등 → `entwurf_v2` native-push delivered ×2 → 수신·모델 연속성 확인(**D0~D6 실증**, D7 partial=수동 tmux 관찰, D8 live 미실증), dead fire→`native-push-target-dead (observed: dead)`·owned→`native-push-no-resume-authority` 정직 reject, mailbox/marker/pi-domain 침범 관찰 0. **단 오라클 agy 배선은 여전히 dangling** — 앞서 "수선 완료" 관측(19시경)은 agent-config working-tree 일시 편집이었고 되돌려짐(antigravity/ 커밋 없음, status clean=구버전). `doctor-agy-bridge`가 이를 정확히 static FAIL로 잡음(#45의 `?` 실물 작동 — L6). **⑨ 공식 smoke 전 GLG 결정 2건: agent-config 배선(공유 server.json 수정 vs 오라클 디바이스 변형 신설) + 라이브 테스트 잔여 meta-record `20260704T201811-071ba8` 정리 방침.** ⑨ smoke 형상은 GPT V5 제안 수령(doctor-static preflight FAIL 게이트 포함, smoke-소유 tmux만 kill, route-trace env 검토). 잔여 선택(blocker 아님): Q14 adapter.ts root/scripts tsconfig 이중 포함은 green이라 그대로(type-only `.ts` import은 root emit-config도 통과 — entwurf-v2-contract.ts 선례 동일; 문서화로 충분).
