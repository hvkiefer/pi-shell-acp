...
> **남은 순서:** ① **GPT가 `LIVE=1 ./run.sh release-gate <scratch>` 실행**(GLG가 GPT에 지정). C1b로 PASS count 18→19 가능 — 최종 live run 후 CHANGELOG/VERIFY 숫자 확정. ② GLG 컷(push + prepare/make-release). **push/tag는 GLG 결정 전 금지.**
